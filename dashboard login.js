...
(function() {
  ...
  // This can be enhanced by session/localStorage
  const user = new URLSearchParams(window.location.search).get('user');

  if (user) {
    document.querySelector('.topbar h2').textContent = `${user} Dashboard`;
    document.querySelector('.user-profile img').src = user === 'Admin' ? 'admin.png' : 'pushkar.png';
  }